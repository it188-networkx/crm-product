...
- **统一反馈分类法**：E-01 F-04、E-02 F-03、E-03 F-03 涉及的用户反馈统一采用
  三级标签体系——"有用 / 不相关 / 已过时"，各 Epic 不得自行扩展或重命名标签；
  采集入口按场景区分（录入侧、检索侧、AI 引用侧），但标签定义与数据结构统一，
  汇总至同一内容运营看板消费。
- **审计框架**：所有 Epic 涉及的操作审计（录入、审核、脱敏、检索反馈、AI 引用反馈）
  统一遵循 GC-04 约束：审计日志不可删除、不可篡改，保留期限 ≥ 12 个月，
  支持按操作者、时间范围、操作类型检索。各 Epic 不再重复定义审计日志基础规则，
  仅在 BR 中声明本 Epic 特有的审计事件类型。
- **权限隔离框架**：所有 Epic 涉及的数据访问权限统一遵循 GC-02 数据主权约束
  与事业部隔离原则，各 Epic 不再重复定义权限过滤基础规则，
  仅在 BR 中声明本 Epic 特有的可见性隔离维度。
