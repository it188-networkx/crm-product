...
- E-07 数据接入是 E-01 线索来源的上游依赖，接入失败时“手工导入”作为降级方案。- E-07 渠道接入负责原始数据拉取与格式映射，E-01 负责映射后数据的去重校验与入池；
  E-07 输出的数据须符合 E-01 F-01 定义的线索数据结构，由 E-01 统一执行去重入池逻辑，两者不重复实现去重能力。
...
> 各 Epic 对 Theme 级 KPI 的贡献为协同关系，非独立达成。下表明确每个 Epic 在 KPI 达成链路中的具体贡献环节。

| KPI | E-07 | E-01 | E-06 | E-05 | E-02 | E-03 | E-04 |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| 月均有效跟进 ≥ 100 家 | 提供多渠道线索供给 | 去重入池扩大有效线索基数 | 标签分类提升分级准确度 | 自动化规则扩大触达覆盖 | 节奏计划保障跟进不遗漏 | 阶段门控防止商机停滞 | 回流优化提升长期转化率 |
| 单次跟进备案 ≤ 30 分钟 | 行为数据自动映射替代手工录入 | — | — | 重复动作自动触发减免人工 | 标准化模板降低记录填写工时 | — | — |
| 年化人力节省 ≥ ¥50 万 | 数据搬运自动化 | 去重自动化 | — | 培育半自动化 | 记录效率提升 | 管理介入精准化 | 策略回流减少试错成本 |
...
- [x] E-05、E-06、E-07 的定义文档已创建，链接已闭合。
