...
| :--- | :--- | :--- | :--- | :--- | :--- |
| F-01 | R1 | 一般 | `marketing-automation/README.md:功能特性` | Epic 功能特性表中 [F-03] 规则运行监控（P1）关联文档标注"待创建"，对应子目录不存在；业务场景三 Step 1（MKT Leader 查看规则运行报告以识别冷却时长问题）无任何 A0304 文档承接，属于 Epic 引用但无文档的悬空 Feature。 | 在当前迭代为 F-03 创建 A0304 文档骨架（含范围声明），或在 Epic README 功能特性表备注中明确 F-03 计划进入哪个迭代，并在场景三 Step 1 标注"依赖 F-03 待创建"以消除悬空引用。 |
| F-02 | R2 | 一般 | `marketing-automation/README.md:业务规则 [BR-01]` / `rule-config/README.md:用户故事` | Epic [BR-01] 规定"标签字典变更时须校验已有规则条件兼容性（承接 E-06 约束）"，该要求为已激活规则的被动失效检测。F-01 仅覆盖发布时的主动校验（AC-04：发布前校验触发条件是否使用有效标签）；F-02 仅覆盖执行时的降级处理（BR-02：废弃标签时跳过本次匹配并生成异常记录）；两个 Feature 均未通过 US 与 AC 覆盖"当 E-06 标签字典发生变更时，对已激活规则触发兼容性校验并通知规则管理员"的场景。 | 在 `rule-config/README.md` 的用户故事中补充 US：作为 MKT Leader，当 E-06 中我配置的触发条件或退出条件所用标签被废弃时，收到系统提示并可更新规则配置，关联验收标准说明系统如何识别该标签变更并推送提示。 |
